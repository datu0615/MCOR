...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, CBAM, C2f,
                 C3, C3TR]:
            if m is Focus:
                c1, c2 = 3, args[0]
                # print("focus c2", c2)
            ...
            c2 = ch[f[0]]  # CIC 모듈의 입력 채널 수 설정
            args = [c2]  # CIC 모듈에 필요한 인자 전달
        elif m is MACSA:
            c2 = ch[f[0]]  # CIC 모듈의 입력 채널 수 설정
            args = [c2]  # CIC 모듈에 필요한 인자 전달
        elif m is MADL:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./models/transformer/yolov5l_fusion_jbx3_ia.yaml', help='model.yaml')
